...
def train(args):
    ...
    optimizer_gen = Adam(model.parameters(), lr = training_config['autoencoder_lr'],  betas = (0.5, 0.999))
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(data_loader):
            ...
            # MODIFIED IMAGE VISUALIZATION LOGIC - ORGANIZED BY CLASS
            if step_count % image_save_steps == 0 or step_count == 1:
                ...
                # DISPLAY IN COLAB - ORGANIZED BY CLASS
                clear_output(wait=True)
                
                if labels is not None:
                    # Get unique classes in current batch
                    unique_classes = np.unique(labels[:sample_size])
                    ...
                    fig.suptitle(f'Epoch {epoch+1}, Step {step_count}', fontsize=16, y=0.995)
                    
                    for idx, class_label in enumerate(unique_classes):
                        # Find indices of this class in the batch
                        class_indices = np.where(labels[:sample_size] == class_label)[0]
                        ...
                        if len(class_indices) > 0:
                            ...
                            # Original image
                            orig_img = save_input[class_idx].permute(1, 2, 0).numpy()
                            if orig_img.shape[2] == 1:  # Grayscale
                                orig_img = orig_img.squeeze()
                                axes[idx, 0].imshow(orig_img, cmap='gray')
                            else:  # RGB
                                axes[idx, 0].imshow(orig_img)
                            axes[idx, 0].set_title(f'Class {class_label} - Original')
                            axes[idx, 0].axis('off')
                            
                            # Reconstructed image
                            recon_img = save_output[class_idx].permute(1, 2, 0).numpy()
                            ...
                            axes[idx, 1].set_title(f'Class {class_label} - Reconstructed')
                            axes[idx, 1].axis('off')
                    
                    plt.tight_layout()
                    plt.show()
                else:
                    # If no labels available, show regular grid
                    fig, ax = plt.subplots(1, 1, figsize=(12, 6))
                    ax.imshow(np.transpose(grid.numpy(), (1, 2, 0)))
                    ax.set_title(f'Epoch {epoch+1}, Step {step_count}\nTop: Original | Bottom: Reconstructed')
                    ax.axis('off')
                    plt.tight_layout()
                    plt.show()
        ...
        if not os.path.exists(os.path.join(training_config['task_name'],training_config['vqvae_autoencoder_ckpt_name'])):
                    os.mkdir(os.path.join(training_config['task_name'], training_config['vqvae_autoencoder_ckpt_name'])) 
        
        if not os.path.exists(os.path.join(training_config['task_name'],training_config['vqvae_discriminator_ckpt_name'])):
                    os.mkdir(os.path.join(training_config['task_name'], training_config['vqvae_discriminator_ckpt_name']))
