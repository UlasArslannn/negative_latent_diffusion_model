    ...
    for epoch_idx in range(num_epochs):
        ...
        for data in tqdm(data_loader):
            ...
            if 'class' in condition_types:
                ...
                class_tensor = cond_input['class'].to(device)
                
                if class_tensor.dim() == 1:
                    # It's class indices, need to convert to one-hot
                    class_indices = class_tensor.tolist()
                    class_condition = torch.nn.functional.one_hot(
                        class_tensor.long(), num_classes
                    ).float()
                else:
                    # It's already one-hot encoded
