...
class Unet(nn.Module):
    ...
    def __init__(self, im_channels, model_config):
        ...
        if self.class_cond:
            ...
            self.class_emb = nn.Embedding(self.num_classes,
                                          self.t_emb_dim)
            # Avoid embedding for negative conditioning
            self.avoid_emb = nn.Embedding(self.num_classes,
                                          self.t_emb_dim)
    ...
    def forward(self, x, t, cond_input=None):
        ...
        if self.class_cond:
            ...
            # Avoid conditioning - subtract avoid embedding from time embedding
            if 'avoid' in cond_input and cond_input['avoid'] is not None:
                avoid_embed = einsum(cond_input['avoid'].float(), self.avoid_emb.weight, 'b n, n d -> b d')
                t_emb -= avoid_embed
        ####################################
